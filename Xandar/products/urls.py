...
app_name = 'products'
urlpatterns = [
    re_path(r'^category=(?P<category>\w+)(&&sub=(?P<sub>[\w-]+)|&&price=(?P<price>\d+)|&&brand=(?P<brand>[\w\s]+)|&&color=(?P<color>\w+)|&&text=(?P<text>[A-Za-z0-9\s]+))*', views.filter_by, name="product_filter"),
